	...
	this.meta_to_tiles = function(meta, sw, sh) {
		...
		for(var y = 0; y < sh; y++) {
			for(var x = 0; x < sw; x++) {
				...
				// Center
				tiles[tx + (ty*self.w)] = new Tile(tx, ty, tile.solid?Content.wall:Content.floor, tile.solid);

				// Right
				tiles[(tx + 1) + (ty*self.w)] = new Tile(tx + 1, ty, tile.wr?Content.wall:Content.floor, tile.wr);
				// Bottom Right
				tiles[(tx + 1) + ((ty + 1)*self.w)] = new Tile(tx + 1, ty + 1, tile.wdr?Content.wall:Content.floor, tile.wdr);
				// Bottom
				tiles[tx + ((ty + 1)*self.w)] = new Tile(tx, ty + 1, tile.wd?Content.wall:Content.floor, tile.wd);

				if(x == 0) {
					// Left
					tiles[(tx - 1) + (ty*self.w)] = new Tile(tx - 1, ty, tile.wl?Content.wall:Content.floor, tile.wl);
					// Bottom Left
					tiles[(tx - 1) + ((ty + 1)*self.w)] = new Tile(tx - 1, ty + 1, tile.wdl?Content.wall:Content.floor, tile.wdl);
				}

				if(y == 0) {
					// Top
					tiles[tx + ((ty - 1)*self.w)] = new Tile(tx, ty - 1, tile.wu?Content.wall:Content.floor, tile.wu);
					// Top Right
					tiles[(tx + 1) + ((ty - 1)*self.w)] = new Tile(tx + 1, ty - 1, tile.wur?Content.wall:Content.floor, tile.wur);
				}

				if(x == 0 && y == 0) {
					// Top Left
					tiles[(tx - 1) + ((ty - 1)*self.w)] = new Tile(tx - 1, ty - 1, tile.wul?Content.wall:Content.floor, tile.wul);
				}
			}
		}
